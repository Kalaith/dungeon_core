...
export const SpeciesSelectionModal: React.FC<SpeciesSelectionModalProps> = ({ open, onClose }) => {
  ...
  const availableSpecies = React.useMemo(() => {
    const speciesMap = new Map<
      string,
      { name: string; monsters: MonsterType[]; description: string }
    >();
      ...
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        <div className="mb-4">
          <h2 className="text-2xl font-bold text-gray-800 mb-2">
            Choose Your Starting Monster Species
          </h2>
          <p className="text-gray-600">
            Select one species to begin your dungeon <strong>(FREE for your first species)</strong>.
            You can unlock additional species later using gold.
          </p>
        </div>
              ...
              <div className="monsters-preview">
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {species.monsters.length > 3 && (
                    <div className="text-xs text-gray-500">+{species.monsters.length - 3} more</div>
                  )}
                </div>
              </div>
            </div>
