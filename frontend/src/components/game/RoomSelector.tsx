...
export const RoomSelector: React.FC = () => {
  ...
  const getNextRoomCost = async () => {
    ...
    const totalRoomCount = floors.reduce((total: number, floor: DungeonFloor) => {
      return (
        total +
        floor.rooms.filter((room: Room) => room.type !== 'core' && room.type !== 'entrance').length
      );
    }, 0);
      ...
      try {
        ...
        if (!deepestFloor) {
          ...
          if (nonCoreRooms.length >= gameConstants.MAX_ROOMS_PER_FLOOR + 1) {
            ...
            const roomType =
              nextPosition === gameConstants.MAX_ROOMS_PER_FLOOR - 1 ? 'boss' : 'normal';
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg">
      ...
      <div className="construction-options space-y-4">
        ...
        <div className="room-info bg-blue-900 p-3 rounded">
          ...
          <div className="text-xs text-blue-300 space-y-1">
            ...
            <div>• {(gameConstants?.MAX_ROOMS_PER_FLOOR ?? 0) + 1} rooms per floor</div>
      ...
      <div className="mt-4 p-2 bg-gray-700 rounded text-xs text-gray-300">
        ...
        {(gameConstants?.MAX_ROOMS_PER_FLOOR ?? 0) + 1} rooms, the next room will start a new floor.
      </div>
    </div>
  );
};
